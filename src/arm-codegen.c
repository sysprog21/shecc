...
void code_generate(void)
{
     ...
     *
     * Since the previous __stmdb instruction pushes nine registers onto
     * stack, set ofs to align(GLOBAL_FUNC->stack_size, 8) + 4 to ensure
     * the stack remains 8-byte aligned after adjustment.
     */
    ofs = ALIGN_UP(GLOBAL_FUNC->stack_size, MIN_ALIGNMENT) + 4;
    emit(__movw(__AL, __r8, ofs));
    emit(__movt(__AL, __r8, ofs));
    emit(__sub_r(__AL, __sp, __sp, __r8));
